...
public class Spline extends Applet {
	...
	public void paint(Graphics g) {
		...
		if (poly) {
			...
			for (int i = 0; i < numpoints - 1; i++) {
				...
				offscreenG.drawLine((int) (.3333 * control[i].x + .6667 * control[i + 1].x),
						...
						(int) (.3333 * control[i].y + .6667 * control[i + 1].y), points[i + 1].x, points[i + 1].y);
			}
		}
		...
		x1 = (int) (1.6077 * (double) points[1].x - .26794 * (double) points[0].x - .43062 * (double) points[2].x
				+ .11483 * (double) points[3].x - .028708 * (double) points[4].x + .004785 * (double) points[5].x);
		y1 = (int) (1.6077 * (double) points[1].y - .26794 * (double) points[0].y - .43062 * (double) points[2].y
				+ .11483 * (double) points[3].y -

				.028708 * (double) points[4].y + .004785 * (double) points[5].y);

		control[1] = new Point(x1, y1);
		x1 = (int) (-.43062 * (double) points[1].x + .07177 * (double) points[0].x + 1.7225 * (double) points[2].x
				- .45933 * (double) points[3].x + .11483 * (double) points[4].x - .019139 * (double) points[3].x);

		y1 = (int) (-.43062 * (double) points[1].y + .07177 * (double) points[0].y + 1.7225 * (double) points[2].y
				- .45933 * (double) points[3].y + .11483 * (double) points[4].y - .019139 * (double) points[3].y);

		control[2] = new Point(x1, y1);
		x1 = (int) (.11483 * (double) points[1].x - .019139 * (double) points[0].x - .45933 * (double) points[2].x
				+ 1.7225 * (double) points[3].x - .43062 * (double) points[4].x + .07177 * (double) points[5].x);

		y1 = (int) (.11483 * (double) points[1].y - .019139 * (double) points[0].y - .45933 * (double) points[2].y
				+ 1.7225 * (double) points[3].y - .43062 * (double) points[4].y + .07177 * (double) points[5].y);

		control[3] = new Point(x1, y1);
		x1 = (int) (-.028708 * (double) points[1].x + .004785 * (double) points[0].x + .114835 * (double) points[2].x
				- .43062 * (double) points[3].x + 1.6077 * (double) points[4].x - .26794 * (double) points[5].x);

		y1 = (int) (-.028708 * (double) points[1].y + .004785 * (double) points[0].y + .114835 * (double) points[2].y
				- .43062 * (double) points[3].y + 1.6077 * (double) points[4].y - .26794 * points[5].y);
		...
		for (int i = 1; i < numpoints; i++) {
			for (t = i - 1; t <= i; t += k) {
				x2 = (int) ((double) points[i - 1].x + (t - (i - 1)) * (-3 * (double) points[i - 1].x
						...
						+ (t - (i - 1)) * (3 * (double) points[i - 1].x
								- 6 * (.6667 * (double) control[i - 1].x + .3333 * (double) control[i].x)
				...
				y2 = (int) ((double) points[i - 1].y + (t - (i - 1)) * (-3 * (double) points[i - 1].y
						...
						+ (t - (i - 1)) * (3 * (double) points[i - 1].y
								- 6 * (.6667 * (double) control[i - 1].y + .3333 * (double) control[i].y)
